    ...
    public function browse(Request $request)
    {
        try {
            ...
            foreach ($tables as $key => $value) {
                if (! in_array($value["name"], $protected_tables)) {
                    // add table watch config
                    $config_watch_tables->addWatchTable($value["name"]);

                    // end add table watch config

                    $table_with_crud_data = [];
                    $table_with_crud_data['table_name'] = $value["name"];
                    $table_with_crud_data['crud_data'] = Badaso::model('DataType')::where('name', $value["name"])->first();
        ...
        try {
            $request->validate([
                ...
                'rows.*.field' => [
                    'required',
                    function ($attribute, $value, $fail) use ($request) {
                        if (! Schema::getColumns($request->name, $value)) {
                            $split_attribute = explode('.', $attribute);
                            $split_attribute[2] = 'relation_type';
                            $field_to_relation = join('.', $split_attribute);
                            if (! $field_to_relation == 'belongs_to_many') {
